...
//LM:#if !defined(_STLPORT_VERSION) && defined(_MSC_VER) && ((_MSC_VER >= 1300) || defined(__INTEL_COMPILER))
...
template <typename T, typename HashFunc = boost::custom_hash<T>, typename EqualKey = std::equal_to<T> >

class ItemMap {
    public:
        typedef T      item_type;
        typedef size_t id_type;
//LM:        typedef hash_map <T, id_type, HashFunc, EqualKey>         hash_map_type;
        typedef boost::unordered_map<T, id_type, HashFunc, EqualKey> hash_map_type;
