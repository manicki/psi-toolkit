...
{
    std::pair<
	typename chart_type::vertex_iterator,
	typename chart_type::vertex_iterator> 
	vertex_iterators = chart_.vertices();

    typename chart_type::vertex_iterator vertex_it = vertex_iterators.first;

    S initial_min_score = combinator_.get_limit_min_score(0);

    for (; vertex_it != vertex_iterators.second; ++vertex_it) {
		std::pair<
			typename chart_type::out_edge_iterator,
			typename chart_type::out_edge_iterator
		> edge_iterators = chart_.out_edges(*vertex_it);

		typename chart_type::out_edge_iterator edge_it = edge_iterators.first;

		for (; edge_it != edge_iterators.second; ++edge_it) {
		    if (chart_.edge_score(*edge_it) >= initial_min_score
			&& combinator_.is_useful(chart_.edge_category(*edge_it))) {
			agenda_.push(*edge_it);
		    }
		}
    }
}
