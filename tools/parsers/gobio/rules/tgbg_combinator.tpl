...
{
    typename hash_wrapper<bid_key, bid_info>::type bids;
    ...
    typename hash_wrapper<int, int>::type reserved_for_temps_hash;

    // i to samo dla extra-atrybutów
    typename hash_wrapper<int, int>::type reserved_for_extra_temps_hash;
	...
	typename hash_wrapper<int,int>::type attrs_to_ts_hash;
	...
	typename hash_wrapper<int,int>::type extra_attrs_to_ts_hash;
	create_reindex_hash_(
...
template<class T, class S, class M, class X, class E>
typename hash_wrapper<typename tgbg_combinator<T,S,M,X,E>::bid_key,
...
void tgbg_combinator<T,S,M,X,E>::create_reindex_hash_(
    ...
    typename hash_wrapper<int,int>::type & r_hash,
    int already_reserved_for_temps)
{
    int temp_ix = already_reserved_for_temps;
    int prev_ix = -1L;
...
void tgbg_combinator<T,S,M,X,E>::compile_rule_(int rule_ix)
{
    ...
    {
	typename hash_wrapper<std::pair<int,int>, int>::type rhs_hash;
