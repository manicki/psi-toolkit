...
//                delete actions;
    ...
    if (ruleName != "")
    {
        ...
        ActionsPtr actions = this->compileRuleAction(chars, ruleLeftSize,
                ruleMatchSize, ruleRightSize, ruleName, repeat);
...
//            repeat = true;
            ...
            action = GroupActionPtr( new GroupAction(groupType, rule->getLeftCount(), (rule->getLeftCount() + rule->getMatchCount() - 1), groupHead, rule->getName()) );
            if (verbose)
                action->setVerbose();
        }
        else if (RE2::FullMatch(actionString, regJoin, &groupType, &groupHead)) {
            action = JoinActionPtr( new JoinAction(groupType, rule->getLeftCount(), (rule->getLeftCount() + rule->getMatchCount() - 1), groupHead, rule->getName()) );
            if (verbose)
                action->setVerbose();
        }
        else if (RE2::FullMatch(actionString, regAttach, &groupType, &groupHead)) {
            action = AttachActionPtr(new AttachAction(groupType, rule->getLeftCount(), (rule->getLeftCount() + rule->getMatchCount() - 1), groupHead, rule->getName()));
            if (verbose)
                action->setVerbose();
        }
        else if (RE2::FullMatch(actionString, regTransform, &groupHead, &groupType)) {
            action = TransformActionPtr( new TransformAction(groupType, groupHead, rule->getName()) );
            if (verbose)
                action->setVerbose();
        }
            ...
            action = DeleteActionPtr( new DeleteAction(conditions, tokenIndex, uncompiled) );
            if (verbose)
                action->setVerbose();
 //           std::cout << "Usun interpretacje: " << container[4] << " pod warunkiem, ze: " << container[1] << " " << container[2] << " " << container[3] << std::endl;
        }
