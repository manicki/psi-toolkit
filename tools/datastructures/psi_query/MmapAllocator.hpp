...
namespace psi {
    ...
    class MmapAllocator {
      ...
      public:
        ...
        MmapAllocator() throw()
         : m_fixed(false), m_fc(m_fixed), m_file_ptr(std::tmpfile(), m_fc),
           m_file_desc(fileno(m_file_ptr.get())),
        ...
        MmapAllocator(std::FILE* f_ptr) throw()
         : m_fixed(false), m_fc(m_fixed), m_file_ptr(f_ptr, m_fc),
           m_file_desc(fileno(m_file_ptr.get())),
           m_page_size(sysconf(_SC_PAGE_SIZE)), m_map_size(0), m_data_ptr(0),
           m_data_size(0), m_data_offset(0)
        { }
        ...
        MmapAllocator(std::FILE* f_ptr, size_t data_size, size_t data_offset = 0) throw()
         : m_fixed(true), m_fc(m_fixed), m_file_ptr(f_ptr, m_fc),
           m_file_desc(fileno(m_file_ptr.get())),
           m_page_size(sysconf(_SC_PAGE_SIZE)), m_map_size(0), m_data_ptr(0),
        ...
        MmapAllocator(std::string fileName) throw()
         : m_fixed(false), m_fc(m_fixed), m_file_ptr(std::fopen(fileName.c_str(), "wb+"), m_fc),
           m_file_desc(fileno(m_file_ptr.get())),
           m_page_size(sysconf(_SC_PAGE_SIZE)), m_map_size(0), m_data_ptr(0),
           m_data_size(0), m_data_offset(0)
        { }
