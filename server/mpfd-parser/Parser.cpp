...
void MPFD::Parser::_ParseHeaders(std::string headers) {
    ...
    // Find name
    unsigned long name_pos = headers.find("name=\"");
    if (name_pos == std::string::npos) {
        throw Exception(std::string("Accepted headers of field does not contain \"name=\".\nThe headers are: \"") + headers + std::string("\""));
    } else {
        unsigned long name_end_pos = headers.find("\"", name_pos + 6);
        ...
        unsigned long filename_pos = headers.find("filename=\"");
        if (filename_pos == std::string::npos) {
            ...
            unsigned long filename_end_pos = headers.find("\"", filename_pos + 10);
            ...
            unsigned long content_type_pos = headers.find("Content-Type: ");
            if (content_type_pos != std::string::npos) {
                long content_type_end_pos = 0;
                for (unsigned int i = content_type_pos + 14; (i < headers.length()) && (!content_type_end_pos); i++) {
